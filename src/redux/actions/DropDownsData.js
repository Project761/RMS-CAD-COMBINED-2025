...
export const getDropDownReasonCase = (LoginAgencyID) => async (dispatch) => {
    const val = { AgencyID: LoginAgencyID }
    fetchPostData('ReasonCase/GetDropDownReasonCase', val).then((data) => {
        if (data) {
            dispatch({ type: ReasonCase_Drp_Data, payload: data });
        } else {
            dispatch({ type: ReasonCase_Drp_Data, payload: [] });
        }
    })
}
