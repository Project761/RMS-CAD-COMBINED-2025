...
export const get_ArresteeName_Data = (openPage, MasterNameID, IncidentID, includeIsArrest = false, arrestID) => async (dispatch) => {
    ...
    fetchPostData('Arrest/GetDataDropDown_Arrestee', openPage ? val1 : val).then((data) => {
        if (data) {
            ...
            dispatch({ type: Arrestee_Name_DrpData, payload: sixColArrayArrest(data, 'NameID', 'Arrestee_Name', 'LastName', 'DateOfBirth', 'Gendre_Description', 'Race_Description', 'NameID', 'MasterNameID', 'AgeFrom', 'IsJuvenile') });
        } else {
            dispatch({ type: Arrestee_Name_DrpData, payload: sixColArrayArrest(data, 'NameID', 'Arrestee_Name', 'LastName', 'DateOfBirth', 'Gendre_Description', 'Race_Description', 'NameID', 'MasterNameID', 'AgeFrom', 'IsJuvenile') });
        }
    })
