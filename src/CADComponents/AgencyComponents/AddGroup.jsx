...
import { AddDeleteUpadate, fetchPostData, fieldPermision, ScreenPermision } from '../../Components/hooks/Api';
import DeletePopUpModal from '../../Components/Common/DeleteModal';
import { Decrypt_Id_Name, base64ToString, tableCustomStyles } from '../../Components/Common/Utility';
import { toastifyError, toastifySuccess } from '../../Components/Common/AlertMsg';
import { AgencyContext } from '../../Context/Agency/Index';
import { Agency_Field_Permistion_Filter } from '../../Components/Filter/AgencyFilter';
import { get_LocalStoreData } from '../../redux/actions/Agency';
import ChangesModal from '../../Components/Common/ChangesModal';
import ReportWorkflow from './ReportWorkflow';

const AddGroup = ({ showModal, setShowModal, get_Group_List }) => {
                ...
                <div className="row ">
                    ...
                    <div className="col-12 col-md-12 col-lg-5 mt-2 ml-lg-5 pl-lg-5 ml-md-1 pl-md-0">
                        <input type="checkbox" name="IsAllowSeal" checked={value.IsAllowSeal} value={value.IsAllowSeal}
                            ...
                            id="IsAllowSeal" />
                        <label className='ml-2' htmlFor="IsAllowSeal">Is Allow Seal</label>
                    </div>
                    ...
                    <div className="col-12 col-md-12 col-lg-5 mt-2 ml-lg-5 pl-lg-5 ml-md-1 pl-md-0 ">
                        <input type="checkbox" name="IsAllowUnSeal" checked={value.IsAllowUnSeal} value={value.IsAllowUnSeal}
                            ...
                            id="IsAllowUnSeal" />
                        <label className='ml-2' htmlFor="IsAllowUnSeal">Is Allow UnSeal</label>
                    </div>
                                ...
                                <div className="row mb-3">
                                    <div className="col-12">
                                        ...
                                        <input
                                            type="text"
                                            className={
                                                `form-control ${fieldPermissionAgency?.GroupName[0] ? fieldPermissionAgency?.GroupName[0]?.Changeok === 0 && fieldPermissionAgency?.GroupName[0]?.AddOK === 0 && status ? 'readonlyColor' : fieldPermissionAgency?.GroupName[0]?.Changeok === 0 && fieldPermissionAgency?.GroupName[0]?.AddOK === 1 && groupEditData?.GroupName === '' && status ? 'requiredColor' : fieldPermissionAgency?.GroupName[0]?.AddOK === 1 && !status
                                                    ? 'requiredColor' : fieldPermissionAgency?.GroupName[0]?.Changeok === 1 && status ? 'requiredColor' : 'readonlyColor' : ''
                                                }`
                                            }
                                            onChange={
                                                fieldPermissionAgency?.GroupName[0]
                                                    ? fieldPermissionAgency?.GroupName[0]?.Changeok === 0 && fieldPermissionAgency?.GroupName[0]?.AddOK === 0 &&
                                                        status ? '' : fieldPermissionAgency?.GroupName[0]?.Changeok === 0 && fieldPermissionAgency?.GroupName[0]?.AddOK === 1 && groupEditData?.GroupName === '' && status ? handlChange : fieldPermissionAgency?.GroupName[0]?.AddOK === 1 && !status ? handlChange : fieldPermissionAgency?.GroupName[0]?.Changeok === 1 && status ? handlChange : '' : handlChange
                                            }
                                            value={value.GroupName}
                                            name="GroupName"
                                            required
                                ...
                                <div className="row mb-4">
                                    <div className="col-12">
                                        ...
                                        <input type="text" name='level'
                                            ...
                                            autocomplete="off" className='form-control'
                                            onChange={handleChange}
                                            maxLength={1} placeholder='Group Name'
