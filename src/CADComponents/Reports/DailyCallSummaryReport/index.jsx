...
import { getData_DropDown_IncidentDispositions, getData_DropDown_Zone } from '../../../CADRedux/actions/DropDownsData';
import { get_ScreenPermissions_Data } from '../../../redux/actions/IncidentAction';

const DailyCallSummaryReport = () => {
    ...
    const [dailyCallSummaryData, setDailyCallSummaryData] = useState([]);
    ...
    const [
        dailyCallSummaryState,
        ,
        handleDailyCallSummaryState,
        clearDailyCallSummaryState,
    ] = useObjState({
        reportedFromDate: "",
        reportedToDate: "",
        zone: "",
    ...
    useEffect(() => {
        if (localStoreData) {
            ...
            dispatch(get_ScreenPermissions_Data("CE109", localStoreData?.AgencyID, localStoreData?.PINID));
    ...
    const getDailyCallSummaryData = async (isPrintReport = false) => {
        setLoader(true);
        if (
            !dailyCallSummaryState?.reportedFromDate &&
            !dailyCallSummaryState?.reportedToDate &&
            !dailyCallSummaryState?.zone?.code &&
            !dailyCallSummaryState?.CADEventFrom &&
            !dailyCallSummaryState?.CADEventTo &&
            !dailyCallSummaryState?.shift &&
            !dailyCallSummaryState?.FoundCFSCodeID &&
            !dailyCallSummaryState?.CADDisposition
        ) {
        ...
        const payload = {
            "AgencyID": loginAgencyID,
            "CADIncidentNumberFrom": dailyCallSummaryState?.CADEventFrom,
            "CADIncidentNumberTo": dailyCallSummaryState?.CADEventTo,
            "ReportedDate": dailyCallSummaryState?.reportedFromDate ? getShowingMonthDateYear(dailyCallSummaryState?.reportedFromDate) : "",
            "ReportedDateTO": dailyCallSummaryState?.reportedToDate ? getShowingMonthDateYear(dailyCallSummaryState?.reportedToDate) : "",
            "ShiftID": dailyCallSummaryState?.shift?.ShiftId,
            "CADCFSCodeID": dailyCallSummaryState?.FoundCFSCodeID,
            // "ZoneID": dailyCallSummaryState?.zone?.value,
            "IncidentDispositionsID": dailyCallSummaryState?.CADDisposition?.IncidentDispositionsID,
            "ZoneCode": dailyCallSummaryState?.zone?.code,
        }
        setSearchValue(dailyCallSummaryState)
        try {
            const response = await ReportsServices.getDailyCallSummaryReport(payload);
            const data = JSON.parse(response?.data?.data);
            if (data?.Table?.length > 0) {
                setDailyCallSummaryData(data?.Table);
    ...
    const handlePrintClick = () => {
        ...
        setShowFooter(true);
        setTimeout(() => {
            printForm(); getDailyCallSummaryData(true); setShowFooter(false);
        }, 100);
    };
                        ...
                        <div className="col-12 col-sm-12">
                            <div className="card Agency">
                                <div className="card-body pt-3 pb-2" >
                                    <div className="row " style={{ marginTop: '-10px' }}>
                                        <div className="col-12 mt-2">
                                            <fieldset>
                                                <legend>Daily Call Summary</legend>
                                                ...
                                                <div className='row mb-1 mt-2 ml-4'>
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <DatePicker
                                                            ...
                                                            id='startDate'
                                                            onChange={(date) => {
                                                                handleDailyCallSummaryState("reportedFromDate", date);
                                                                if (!date) {
                                                                    handleDailyCallSummaryState("reportedToDate", ""); // Clear reportedToDate if from date is cleared
                                                                }
                                                            }}
                                                            selected={dailyCallSummaryState?.reportedFromDate || ""}
                                                            ...
                                                            // is24Hour
                                                            // timeFormat="HH:mm "
                                                            isClearable={dailyCallSummaryState?.reportedFromDate ? true : false}
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <DatePicker
                                                            ...
                                                            id='reportedToDate'
                                                            onChange={(date) => {
                                                                handleDailyCallSummaryState("reportedToDate", date);
                                                            }}
                                                            selected={dailyCallSummaryState?.reportedToDate || ""}
                                                            dateFormat="MM/dd/yyyy"
                                                            // timeCaption="Time"
                                                            showMonthDropdown
                                                            showYearDropdown
                                                            ...
                                                            // is24Hour
                                                            // timeFormat="HH:mm "
                                                            isClearable={dailyCallSummaryState?.reportedToDate ? true : false}
                                                            showDisabledMonthNavigation
                                                            dropdownMode="select"
                                                            autoComplete="off"
                                                            placeholderText="Select To Date..."
                                                            minDate={dailyCallSummaryState?.reportedFromDate || new Date(1991, 0, 1)}
                                                            maxDate={new Date(datezone)}
                                                            disabled={!dailyCallSummaryState?.reportedFromDate} // Disable if from date is not selected
                                                            className={!dailyCallSummaryState?.reportedFromDate && 'readonlyColor'}
                                                        />
                                                    </div>
                                                    <div className="col-1 d-flex align-self-center justify-content-end">
                                                        <label for="" className="tab-form-label">
                                                    ...
                                                    <div className="col-2 w-100">
                                                        <Select
                                                            ...
                                                            value={dailyCallSummaryState?.zone}
                                                            onChange={(e) => { handleDailyCallSummaryState("zone", e) }}
                                                        />
                                                    </div>
                                                </div>
                                                <div className='row mb-1 mt-2 ml-4'>
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <input
                                                            ...
                                                            value={dailyCallSummaryState.CADEventFrom}
                                                            onChange={(e) => {
                                                                const newValue = e.target.value;
                                                                handleDailyCallSummaryState("CADEventFrom", newValue);
                                                                if (!newValue) {
                                                                    handleDailyCallSummaryState("CADEventTo", ""); // Clear CADEventTo
                                                                }
                                                            }}
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <input
                                                            ...
                                                            value={dailyCallSummaryState.CADEventTo}
                                                            onChange={(e) => handleDailyCallSummaryState("CADEventTo", e.target.value)}
                                                            onKeyDown={handleNumberTextKeyDown}
                                                            disabled={!dailyCallSummaryState.CADEventFrom}
                                                        />
                                                    </div>
                                                    <div className="col-1 d-flex align-self-center justify-content-end">
                                                        <label for="" className="tab-form-label">
                                                    ...
                                                    <div className="col-2 w-100">
                                                        <Select
                                                            ...
                                                            value={dailyCallSummaryState?.shift}
                                                            onChange={(e) => { handleDailyCallSummaryState("shift", e) }}
                                                            onInputChange={(inputValue, actionMeta) => {
                                                                if (inputValue.length > 12) {
                                                                    return inputValue.slice(0, 12);
                                                                }
                                                ...
                                                <div className='row mb-1 mt-2 ml-4'>
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <Select
                                                            name="CFSLId"
                                                            value={CFSDropDown.find((opt) => opt.CallforServiceID === dailyCallSummaryState?.FoundCFSCodeID) || null}  // Keep the selected value
                                                            options={CFSDropDown}
                                                            getOptionLabel={(v) => v?.CFSCODE}  // Show only value after selection
                                                            getOptionValue={(v) => v?.CallforServiceID}
                                                            onChange={(v) => {
                                                                handleDailyCallSummaryState("FoundCFSCodeID", v?.CallforServiceID);
                                                                handleDailyCallSummaryState("FoundCFSLDesc", v?.CallforServiceID);
                                                                handleDailyCallSummaryState("FoundPriorityID", v?.PriorityID);
                                                            }}
                                                            placeholder="Select..."
                                                    ...
                                                    <div className="col-3 d-flex align-self-center justify-content-end">
                                                        <Select
                                                            name="CFSLDesc"
                                                            value={CFSDropDown.find((opt) => opt.CallforServiceID === dailyCallSummaryState?.FoundCFSLDesc) || null}  // Keep the selected value
                                                            options={CFSDropDown}
                                                    ...
                                                    <div className="col-2 w-100">
                                                        <Select
                                                            ...
                                                            className="w-100"
                                                            value={dailyCallSummaryState?.CADDisposition}
                                                            onChange={(e) => handleDailyCallSummaryState("CADDisposition", e)}
                                                            name="CADDisposition" />
                                                    </div>
                                                </div>
                                            </fieldset>
                                ...
                                <div className="col-12 col-md-12 col-lg-12 mt-1 text-right mb-1">
                                    {effectiveScreenPermission?.[0]?.AddOK ? <button className="btn btn-sm bg-green text-white px-2 py-1" onClick={() => { getDailyCallSummaryData(false); }} >Show Report</button> : <></>}
                ...
                <>
                    <div className="col-12 col-md-12 col-lg-12  px-2" >
                        <div className="bg-line  py-1 px-2 mt-1 d-flex justify-content-between align-items-center">
                            <p className="p-0 m-0 d-flex align-items-center">Daily Call Summary Report</p>
                                                    ...
                                                    {dailyCallSummaryData
                                                        .map((obj) => (
                                                            <tr>
                                                                ...
                                                                <td className="text-list">{obj.PatrolZone}</td>
                                                                <td className="text-list"> {obj.CADDisposition}</td>
                                                            </tr>
                                                        ))}
                                                </tbody>
                                            </table>
...
export default DailyCallSummaryReport