...
const OfficerActivityReport = () => {
    ...
    const [
        officerActivityState,
        ,
        handleOfficerActivityState,
        clearOfficerActivityState,
    ] = useObjState({
        reportedFromDate: "",
        reportedToDate: "",
        callTaker: "",
    ...
    useEffect(() => {
        if (localStoreData) {
            ...
            dispatch(get_ScreenPermissions_Data("CE106", localStoreData?.AgencyID, localStoreData?.PINID));
    ...
    const getIncidentSearchData = async (isPrintReport = false) => {
        setLoader(true);
        if (
            !officerActivityState?.reportedFromDate &&
            !officerActivityState?.reportedToDate &&
            !officerActivityState?.callTaker?.PINID &&
            !officerActivityState?.CADEventFrom &&
            !officerActivityState?.CADEventTo &&
            !officerActivityState?.zone?.code &&
            !officerActivityState?.FoundCFSCodeID &&
            !officerActivityState?.shift
        ) {
        ...
        const payload = {
            "AgencyID": loginAgencyID,
            "CADIncidentNumberFrom": officerActivityState?.CADEventFrom,
            "CADIncidentNumberTo": officerActivityState?.CADEventTo,
            "OfficerID": officerActivityState?.callTaker?.PINID,
            "ReportedDate": officerActivityState?.reportedFromDate ? getShowingMonthDateYear(officerActivityState?.reportedFromDate) : "",
            "ReportedDateTO": officerActivityState?.reportedToDate ? getShowingMonthDateYear(officerActivityState?.reportedToDate) : "",
            "ShiftID": officerActivityState?.shift?.ShiftId,
            "CADCFSCodeID": officerActivityState?.FoundCFSCodeID,
            "ZoneID": officerActivityState?.zone?.value,
            // "ZoneCode": officerActivityState?.zone?.code,
            // "CADCFSCodeID":1,
            // "ZoneDescription": officerActivityState?.zone?.label,
        }
        setSearchValue(officerActivityState)
        try {
            const response = await ReportsServices.getOfficerActivityReport(payload);
            const data = JSON.parse(response?.data?.data);
            if (data?.Table?.length > 0) {
                setOfficerActivityData(data?.Table);
                ...
                if (!isPrintReport) {
                    toastifyError("Data Not Available"); setOfficerActivityData([]); setAgencyData([])
                                ...
                                <div className="card-body pt-3 pb-2" >
                                    <div className="row " style={{ marginTop: '-10px' }}>
                                        <div className="col-12 mt-2">
                                            <fieldset>
                                                <legend>Officer Activity</legend>
                                                <div className='row mb-1 mt-2 ml-4'>
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <input
                                                            ...
                                                            value={officerActivityState.CADEventFrom}
                                                            onChange={(e) => {
                                                                const newValue = e.target.value;
                                                                handleOfficerActivityState("CADEventFrom", newValue);
                                                                if (!newValue) {
                                                                    handleOfficerActivityState("CADEventTo", ""); // Clear CADEventTo
                                                                }
                                                            }}
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <input
                                                            ...
                                                            value={officerActivityState.CADEventTo}
                                                            onChange={(e) => handleOfficerActivityState("CADEventTo", e.target.value)}
                                                            onKeyDown={handleNumberTextKeyDown}
                                                            disabled={!officerActivityState.CADEventFrom}
                                                        />
                                                    </div>
                                                    <div className="col-1 d-flex align-self-center justify-content-end">
                                                        <label for="" className="tab-form-label">
                                                    ...
                                                    <div className="col-2 w-100">
                                                        <Select
                                                            ...
                                                            value={officerActivityState?.callTaker}
                                                            isClearable
                                                            onChange={(e) => { handleOfficerActivityState("callTaker", e) }}
                                                        />
                                                    </div>
                                                </div>
                                                <div className='row mb-1 mt-2 ml-4'>
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <DatePicker
                                                            ...
                                                            id='startDate'
                                                            onChange={(date) => {
                                                                handleOfficerActivityState("reportedFromDate", date);
                                                                if (!date) {
                                                                    handleOfficerActivityState("reportedToDate", ""); // Clear reportedToDate if from date is cleared
                                                                }
                                                            }}
                                                            selected={officerActivityState?.reportedFromDate || ""}
                                                            ...
                                                            // is24Hour
                                                            // timeFormat="HH:mm "
                                                            isClearable={officerActivityState?.reportedFromDate ? true : false}
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <DatePicker
                                                            ...
                                                            id='reportedToDate'
                                                            onChange={(date) => {
                                                                handleOfficerActivityState("reportedToDate", date);
                                                            }}
                                                            selected={officerActivityState?.reportedToDate || ""}
                                                            dateFormat="MM/dd/yyyy"
                                                            // timeCaption="Time"
                                                            showMonthDropdown
                                                            showYearDropdown
                                                            ...
                                                            // is24Hour
                                                            // timeFormat="HH:mm "
                                                            isClearable={officerActivityState?.reportedToDate ? true : false}
                                                            showDisabledMonthNavigation
                                                            dropdownMode="select"
                                                            autoComplete="off"
                                                            placeholderText="Select To Date..."
                                                            minDate={officerActivityState?.reportedFromDate || new Date(1991, 0, 1)}
                                                            maxDate={new Date(datezone)}
                                                            disabled={!officerActivityState?.reportedFromDate} // Disable if from date is not selected
                                                            className={!officerActivityState?.reportedFromDate && 'readonlyColor'}
                                                        />
                                                    </div>
                                                    <div className="col-1 d-flex align-self-center justify-content-end">
                                                        <label for="" className="tab-form-label">
                                                    ...
                                                    <div className="col-2 w-100">
                                                        <Select
                                                            ...
                                                            value={officerActivityState?.shift}
                                                            onChange={(e) => { handleOfficerActivityState("shift", e) }}
                                                ...
                                                <div className='row mb-1 mt-2 ml-4'>
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <Select
                                                            name="CFSLId"
                                                            value={CFSDropDown.find((opt) => opt.CallforServiceID === officerActivityState?.FoundCFSCodeID) || null}  // Keep the selected value
                                                            options={CFSDropDown}
                                                            getOptionLabel={(v) => v?.CFSCODE}  // Show only value after selection
                                                            getOptionValue={(v) => v?.CallforServiceID}
                                                            onChange={(v) => {
                                                                handleOfficerActivityState("FoundCFSCodeID", v?.CallforServiceID);
                                                                handleOfficerActivityState("FoundCFSLDesc", v?.CallforServiceID);
                                                                handleOfficerActivityState("FoundPriorityID", v?.PriorityID);
                                                            }}
                                                            placeholder="Select..."
                                                            styles={colorLessStyle_Select}
                                                            className="w-100"
                                                    ...
                                                    <div className="col-3 d-flex align-self-center justify-content-end">
                                                        <Select
                                                            name="CFSLDesc"
                                                            value={CFSDropDown.find((opt) => opt.CallforServiceID === officerActivityState?.FoundCFSLDesc) || null}  // Keep the selected value
                                                            options={CFSDropDown}
                                                    ...
                                                    <div className="col-2 w-100">
                                                        <Select
                                                            ...
                                                            value={officerActivityState?.zone}
                                                            onChange={(e) => { handleOfficerActivityState("zone", e) }}
                                                        />
                                                    </div>
                                                </div>
                                            </fieldset>
                ...
                <>
                    <div className="col-12 col-md-12 col-lg-12  px-2" >
                        <div className="bg-line  py-1 px-2 mt-1 d-flex justify-content-between align-items-center">
                            <p className="p-0 m-0 d-flex align-items-center">Officer Activity Report</p>
                            ...
                            <div className="row" >
                                ...
                                </div>
                                {
                                    <div className='col-12' style={{ pageBreakAfter: 'always', width: "100%" }}>
                                        {[
                                            ...new Set(officerActivityData.map((obj) => obj.OfficerName)),
                                        ].map((OfficerName, index, array) => {
                                            const filteredData = officerActivityData.filter((obj) => obj.OfficerName === OfficerName);
                                            ...
                                            return (
                                                <div key={`${OfficerName}-${index}`}>
                                                    <div className="table-responsive">
                                                        <table className="table table-bordered mb-0">
                                                            <thead className="text-dark master-table text-bold">
                                                                <tr>
                                                                    <th className="" style={{ width: '180px' }}>Officer</th>
                                                                    <th className="" style={{ width: '80px' }}>Totals Calls</th>
                                                                    <th className="" style={{ width: '100px' }}>Shift</th>
                                                                    <th className="" style={{ width: '180px' }}>Patrol Zone</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody className="master-tbody">
                                                                <tr>
                                                                    <td className="text-list" style={{ width: '180px' }}>{commonData.OfficerName}</td>
                                                                    <td className="text-list" style={{ width: '80px' }}>{commonData.TotalDPCalls}</td>
                                                                    <td className="text-list" style={{ width: '100px' }}>{commonData.Shift}</td>
                                                                    <td className="text-list" style={{ width: '180px' }}>{commonData.Zone}</td>
                                                                </tr>
                                                            </tbody>
                                                        </table>
...
export default OfficerActivityReport