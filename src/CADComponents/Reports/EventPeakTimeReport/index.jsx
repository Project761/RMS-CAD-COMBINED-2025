...
const EventPeakTimeReport = () => {
    ...
    const [eventPeakTimeData, setEventPeakTimeData] = useState([]);
    ...
    const [
        eventPeakTimeState,
        ,
        handleEventPeakTimeState,
        clearEventPeakTimeState,
    ] = useObjState({
        reportedFromDate: "",
        reportedToDate: "",
        shift: "",
    ...
    const getEventPeakTimeData = async (isPrintReport = false) => {
        setLoader(true);

        if (
            !eventPeakTimeState?.reportedFromDate &&
            !eventPeakTimeState?.reportedToDate &&
            !eventPeakTimeState?.shift &&
            !eventPeakTimeState?.FoundCFSCodeID &&
            !eventPeakTimeState?.zone?.code
        ) {
        ...
        const payload = {
            "AgencyID": loginAgencyID,
            "REPORTEDDATE": eventPeakTimeState?.reportedFromDate ? getShowingMonthDateYear(eventPeakTimeState?.reportedFromDate) : "",
            "ReportedDateTo": eventPeakTimeState?.reportedToDate ? getShowingMonthDateYear(eventPeakTimeState?.reportedToDate) : "",
            "Shift": eventPeakTimeState?.shift?.ShiftId,
            "CFSID": eventPeakTimeState?.FoundCFSCodeID,
            // "ZoneID": eventPeakTimeState?.zone?.value,
            // "ZoneDescription": eventPeakTimeState?.zone?.label,
            "ZoneCode": eventPeakTimeState?.zone?.code,
        }

        setSearchValue(eventPeakTimeState)

        try {
            const response = await ReportsServices.getEventPeakTimeReport(payload);
            const data = JSON.parse(response?.data?.data);
            if (data?.Table?.length > 0) {
                setEventPeakTimeData(data?.Table);
                ...
                if (!isPrintReport) {
                    toastifyError("Data Not Available"); setEventPeakTimeData([]); setAgencyData([])
    ...
    const handlePrintClick = () => {
        ...
        setShowFooter(true);
        setTimeout(() => {
            printForm(); getEventPeakTimeData(true); setShowFooter(false);
        }, 100);
    };
                                            ...
                                            <fieldset>
                                                ...
                                                <div className='row mb-1 mt-2 ml-4'>
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <DatePicker
                                                            ...
                                                            id='startDate'
                                                            onChange={(date) => {
                                                                handleEventPeakTimeState("reportedFromDate", date);
                                                                if (!date) {
                                                                    handleEventPeakTimeState("reportedToDate", ""); // Clear reportedToDate if from date is cleared
                                                                }
                                                            }}
                                                            selected={eventPeakTimeState?.reportedFromDate || ""}
                                                            ...
                                                            // is24Hour
                                                            // timeFormat="HH:mm "
                                                            isClearable={eventPeakTimeState?.reportedFromDate ? true : false}
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <DatePicker
                                                            ...
                                                            id='reportedToDate'
                                                            onChange={(date) => {
                                                                handleEventPeakTimeState("reportedToDate", date);
                                                            }}
                                                            selected={eventPeakTimeState?.reportedToDate || ""}
                                                            dateFormat="MM/dd/yyyy"
                                                            // timeCaption="Time"
                                                            showMonthDropdown
                                                            showYearDropdown
                                                            ...
                                                            // is24Hour
                                                            // timeFormat="HH:mm "
                                                            isClearable={eventPeakTimeState?.reportedToDate ? true : false}
                                                            showDisabledMonthNavigation
                                                            dropdownMode="select"
                                                            autoComplete="off"
                                                            placeholderText="Select To Date..."
                                                            minDate={eventPeakTimeState?.reportedFromDate || new Date(1991, 0, 1)}
                                                            maxDate={new Date(datezone)}
                                                            disabled={!eventPeakTimeState?.reportedFromDate} // Disable if from date is not selected
                                                            className={!eventPeakTimeState?.reportedFromDate && 'readonlyColor'}
                                                        />
                                                    </div>
                                                    <div className="col-1 d-flex align-self-center justify-content-end">
                                                        <label for="" className="tab-form-label">
                                                    ...
                                                    <div className="col-2 w-100">
                                                        <Select
                                                            ...
                                                            value={eventPeakTimeState?.shift}
                                                            onChange={(e) => { handleEventPeakTimeState("shift", e) }}
                                                ...
                                                <div className='row mb-1 mt-2 ml-4'>
                                                    ...
                                                    <div className="col-2 d-flex align-self-center justify-content-end">
                                                        <Select
                                                            name="CFSLId"
                                                            value={CFSDropDown.find((opt) => opt.CallforServiceID === eventPeakTimeState?.FoundCFSCodeID) || null}  // Keep the selected value
                                                            options={CFSDropDown}
                                                            getOptionLabel={(v) => v?.CFSCODE}  // Show only value after selection
                                                            getOptionValue={(v) => v?.CallforServiceID}
                                                            onChange={(v) => {
                                                                handleEventPeakTimeState("FoundCFSCodeID", v?.CallforServiceID);
                                                                handleEventPeakTimeState("FoundCFSLDesc", v?.CallforServiceID);
                                                                handleEventPeakTimeState("FoundPriorityID", v?.PriorityID);
                                                            }}
                                                            placeholder="Select..."
                                                    ...
                                                    <div className="col-3 d-flex align-self-center justify-content-end">
                                                        <Select
                                                            name="CFSLDesc"
                                                            value={CFSDropDown.find((opt) => opt.CallforServiceID === eventPeakTimeState?.FoundCFSLDesc) || null}  // Keep the selected value
                                                            options={CFSDropDown}
                                                    ...
                                                    <div className="col-2 w-100">
                                                        <Select
                                                            ...
                                                            value={eventPeakTimeState?.zone}
                                                            onChange={(e) => { handleEventPeakTimeState("zone", e) }}
                                                        />
                                                    </div>
                                                </div>
                                            </fieldset>
                ...
                <>
                    <div className="col-12 col-md-12 col-lg-12  px-2" >
                        <div className="bg-line  py-1 px-2 mt-1 d-flex justify-content-between align-items-center">
                            <p className="p-0 m-0 d-flex align-items-center">Event Peak Time Report</p>
                                        ...
                                        <div className="row mt-2">
                                                ...
                                                <>
                                                    ...
                                                    <div className="col-3 col-md-3 col-lg-4 text-field mt-1">
                                                        <input type="text" className='readonlyColor'
                                                            // value={eventPeakTimeState.showReportedFromDate}
                                                            // value={getShowingWithOutTime(eventPeakTimeState.reportedFromDate)}
                                            ...
                                            {showFields.showReportedToDate && (
                                                <>
                                                    ...
                                                    <div className="col-3 col-md-3 col-lg-4 text-field mt-1">
                                                        <input type="text" className='readonlyColor'
                                                            // value={getShowingWithOutTime(eventPeakTimeState.reportedToDate)}
                                    ...
                                    <div className="table-responsive">
                                        <table className="table table-bordered">
                                            <thead className="text-dark master-table text-bold">
                                                <tr>
                                                    <th className="" style={{ width: '300px' }} colSpan="2">CFS Code/Description</th>
                                                    <th className="" >00</th>
                                                    <th className="" >01</th>
                                                    <th className="" >02</th>
                                                    <th className="" >03</th>
                                                    ...
                                                    <th className="" >10</th>
                                                    <th className="" >11</th>
                                                    <th className="" style={{ width: '60px' }} rowSpan="2">Total</th>
                                                </tr>
                                                <tr>
                                        ...
                                        {eventPeakTimeData.map((obj, index) => {
                                            const isLastItem = index === eventPeakTimeData.length - 1;
                                            return (
                                                <div >
                                                    <>
                                                        <table className="table table-bordered" style={{ marginBottom: "0px" }}>
                                                            <tbody className="master-tbody">
                                                                <tr>
                                                                    <td className="text-list" colSpan="2" style={{ width: '300px' }}>{obj.CFSCodeDescription || "-"}</td>
                                                                    <td className="text-list" > {obj[0] || "-"}</td>
                                                                    <td className="text-list" >{obj[1] || "-"}</td>
                                                                    <td className="text-list" >{obj[2] || "-"}</td>
                                                                    <td className="text-list" >{obj[3] || "-"}</td>
                                                                    <td className="text-list" >{obj[4] || "-"}</td>
                                                                    <td className="text-list" >{obj[5] || "-"}</td>
                                                                    <td className="text-list" >{obj[6] || "-"}</td>
                                                                    <td className="text-list" >{obj[7] || "-"}</td>
                                                                    <td className="text-list" >{obj[8] || "-"}</td>
                                                                    <td className="text-list" >{obj[9] || "-"}</td>
                                                                    <td className="text-list" >{obj[10] || "-"}</td>
                                                                    <td className="text-list" > {obj[11] || "-"}</td>
                                                                    <td className="text-list" style={{ width: '60px' }} rowSpan="2">{obj?.Total || "-"}</td>
                                                                </tr>
                                                                <tr>
                                                                    <td className="text-list" style={{ width: '150px' }}>{obj.ShiftDescriptions || "-"}</td>
                                                                    <td className="text-list" style={{ width: '150px' }}>{obj.ZoneDescriptions || "-"}</td>
                                                                    <td className="text-list" >{obj[12] || "-"}</td>
                                                                    <td className="text-list" >{obj[13] || "-"}</td>
                                                                    <td className="text-list" >{obj[14] || "-"}</td>
                                                                    <td className="text-list" >{obj[15] || "-"}</td>
                                                                    <td className="text-list" >{obj[16] || "-"}</td>
                                                                    <td className="text-list" >{obj[17] || "-"}</td>
                                                                    <td className="text-list" >{obj[18] || "-"}</td>
                                                                    <td className="text-list" >{obj[19] || "-"}</td>
                                                                    <td className="text-list" >{obj[20] || "-"}</td>
                                                                    <td className="text-list" >{obj[21] || "-"}</td>
                                                                    <td className="text-list" >{obj[22] || "-"}</td>
                                                                    <td className="text-list" >{obj[23] || "-"}</td>
                                                                </tr>
                                                            </tbody>
                                                        </table>
...
export default EventPeakTimeReport