...
const VerifyLocation = (props) => {
    ...
    const { loginAgencyID, modalStatus, setModalStatus, value, setValue, setChangesStatus, addVerifySingleData, get_Add_Single_Data, setStatesChangeStatus } = props
    ...
    const [locationVal, setLocationVal] = useState({
        'AgencyID': loginAgencyID ? loginAgencyID : '', 'AgencyName': '', 'LocationID': '', 'IsActive': '1', 'PremiseNo': '', 'PremiseNo_Parse': '', 'Street': '', 'Street_Parse': '', 'CommonPlace': '', 'ApartmentNo': '', 'ApartmentType': '', 'CountryID': '', 'Country': '', 'State': '', 'CityID': '', 'City': '', 'ZipCodeID': '', 'ZipCode': '', 'Statefullname': '', 'DirectionPrefix': '', 'DirectionSufix': '', 'DirectionPrefix_Parse': '', 'DirectionSuffix_Parse': '', 'TypeSufix': '', 'TypeSuffix_Parse': '', 'IsUsLocation': 'Y', 'Address': '',
    });
    ...
    const { StreetError, CountryIDError, StateError, CityIDError, StatefullnameError, ApartmentTypeError } = errors
    ...
    const Reset = () => {
        ...
        setErrors({
            ...errors,
            'StreetError': '', 'CountryIDError': '', 'StateError': '', 'CityIDError': '', 'ZipCodeIDError': '', 'ApartmentTypeError': '', 'StatefullnameError': '',
        });
        ...
        AddDeleteUpadate('MasterLocation/Insert_Location', val).then((res) => {
            if (res.success) {
                ...
                if (res.LocationID) {
                    ...
                    setValue({
                        ...value,
                        ['NameLocationID']: parseInt(res.LocationID),
    ...
    const ResetFields_On_Change = (Code) => {
        if (addVerifySingleData[0]?.IsUsLocation === 'Y' && locationVal?.IsUsLocation === 'Y' && addVerifySingleData.length > 0) {
            get_Add_Single_Data(value.NameLocationID);
        } else {
            setLocationVal({
                ...locationVal,
                'PremiseNo': '',
        ...
        <>
            {
                modalStatus ?
                    ...
                    <dialog className="modal fade" style={{ background: "rgba(0,0,0, 0.5)" }} id="NameVerifyModal" tabIndex="-1" aria-hidden="true" data-backdrop="false">
