...
import { Decrypt_Id_Name, Requiredcolour, base64ToString, customStylesWithOutColor, filterPassedDateTime, filterPassedTimeZone, filterPassedTimeZonesProperty, getShowingDateText, getShowingMonthDateYear, getYearWithOutDateTime, stringToBase64 } from '../../../Common/Utility'
    ...
    const Insert_Vehicle = () => {
        ...
        AddDeleteUpadate('PropertyVehicle/Insert_PropertyVehicle', val).then((res) => {
            if (res.success) {
                ...
                if (res?.PropertyID) {
                    navigate(`/Missing-Vehicle-Home?IncId=${IncID}&IncNo=${IncNo}&IncSta=${IncSta}&MissPerID=${MissPerId}&MissPerSta=true&MissVehID=${stringToBase64(res?.PropertyID)}&MissPerSta=true`);
    ...
    const Update_MissingVehicle_Data = () => {
        ...
        AddDeleteUpadate('PropertyVehicle/Update_PropertyVehicle', val).then((res) => {
            if (res.success) {
                ...
                setValue({ ...value, Value: previousValue, }); Insert_MisingPerson_Vehicle(VehicleID); setStatesChangeStatus(false)
            } else {
    ...
    const Delete_MisingPerson_Vehicle = () => {
        ...
        AddDeleteUpadate('MissingPerson/Insert_MissingPersonVehicle', val).then((res) => {
            if (res.success) {
                ...
                navigate(`/Missing-Vehicle-Home?IncId=${IncID}&IncNo=${IncNo}&IncSta=${IncSta}&MissPerID=${MissPerId}&MissPerSta=${true}&MissVehID=${''}&MissPerSta=${true}`)
