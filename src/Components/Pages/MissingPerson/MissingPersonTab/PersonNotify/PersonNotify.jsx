...
const PersonNotify = ({ ListData }) => {
    ...
    console.log("@@@@@missingVehicleID >>", missingVehicleID);
    console.log("@@@@@DecMissVehID >>", DecMissVehID);
    ...
    console.log("^^^^ personNotifyUIForm", personNotifyUIForm)
    useEffect(() => {
    ...
    const check_Validation_Error = (e) => {
        ...
        // Only validate if missingVehicleID exists
        if (missingVehicleID) {
        ...
        // Always call PersonInfo functions (mandatory)


        // Only call Update_MissingVehicle_Data if missingVehicleID exists and all validations pass
        if (missingVehicleID && allValidationsPass) {
    ...
    const getPlateStateStyle = () => {
        return categoryCode === "NMVs" || plateTypeCode === "Unknown" ? WidhoutColorStyles : missingVehicleID ? Requiredcolour : customStylesWithOutColor
    }
    ...
    const ChangeDropDown = (e, name) => {
        console.log("ChangeDropDown e", e, name)
        !addUpdatePermission && setStatesChangeStatus(true); !addUpdatePermission && setChangesStatus(true);
        if (e) {
            if (name === 'MakeID') {
    ...
    const resetPersonNotifyUIForm = () => {
        setPersonNotifyUIForm(initialState)

    }


    // Get Single Data - Missing Person Associated
    const Get_SingleDataPersonInfo = () => {
        const val = { 'MissingPersonID': DecMissPerID, }
        fetchPostData('MissingPersonAssociated/GetData_MissingPersonAssociated', val).then((res) => {
            console.log("^^^ GetData_MissingPersonAssociated res", res[0])
            console.log("^^^ res.length", res.length)
            if (res.length > 0) {
                setAssociatedId(res[0]?.AssociatedReportingInfoID);
                setMissingVehicleID(res[0]?.VehicleID);
                setValue({ ...value, ['VehicleID']: res[0]?.VehicleID });
    ...
    // Insert - Missing Person Associated
    const Insert_PersonInfo = () => {
        const val = {
        ...
        AddDeleteUpadate('MissingPersonAssociated/InsertMissingPersonAssociated', val).then((res) => {
            ...
            if (res.success) {
                ...
                const AssociatedReportingInfoID = parsedData.Table[0].AssociatedReportingInfoID;
                console.log("@@@@AssociatedReportingInfoID", AssociatedReportingInfoID);
                setAssociatedId(AssociatedReportingInfoID);
                Get_SingleDataPersonInfo();
            } else {
    ...
    // Update - Missing Person Associated
    const Update_PersonInfo = () => {
        const val = {
                        ...
                        <div className="row">
                            ...
                            <div className="col-4 col-md-4 col-lg-4 mt-1 ">
                                <Select
                                    styles={customStylesWithOutColor}
                                    name="relationship"
                                    value={missingPersonRelationshipDrpData?.filter((obj) => obj.value === personNotifyUIForm?.relationship)}
                                    options={missingPersonRelationshipDrpData}
                                    onChange={(e) => { handleDropDownChange(e, 'relationship') }}
                            ...
                            <div className="col-10 col-md-10 col-lg-3 ">
                                {
                                    MstVehicle === 'MST-Vehicle-Dash' ?
                                        <DatePicker
                                            ...
                                            disabled={nibrsSubmittedvehicleMain === 1 || !missingVehicleID}
                                            className={`${nibrsSubmittedvehicleMain === 1 ? 'readonlyColor' : 'requiredColor'} ${!missingVehicleID ? 'readonlyColor' : ''}`}
                                        />
                                        :
                                        <DatePicker
                                            id='reportedDtTm'
                        ...
                        <div className="row">
                            ...
                            <div className="col-4 col-md-4 col-lg-3 mt-1" >
                                <Select
                                    ...
                                    styles={missingVehicleID ? Requiredcolour : customStylesWithOutColor}
                                    options={propertyLossCodeData}
                                    onChange={(e) => ChangeDropDown(e, 'LossCodeID')}

                                    isClearable
                                    placeholder="Select..."
                                    isDisabled={!missingVehicleID}
                                    className={`${missingVehicleID ? '' : 'readonlyColor'}`}
                                />
                            </div>
                            <div className="col-2 col-md-2 col-lg-1 mt-2 ">
                            ...
                            <div className="col-4 col-md-4 col-lg-3 mt-1">
                                <Select
                                    ...
                                    isClearable
                                    isDisabled={!value?.CategoryID || !missingVehicleID}
                                    className={`${!value?.CategoryID || !missingVehicleID ? 'readonlyColor' : ''}`}
                                    placeholder="Select..."
                            ...
                            <div className="col-12 col-md-12 col-lg-4 d-flex ">
                                <div className="col-3 col-md-2 col-lg-4 mt-2 pt-1 text-left ">
                                    <label htmlFor="" className='new-label '>
                                        Plate&nbsp;State&nbsp;&&nbsp;No.
                                        {errors.PlateStateNoError !== 'true' && missingVehicleID ? (
                                ...
                                <div className="col-4 col-md-6 col-lg-5 mt-1" >
                                    <Select
                                        ...
                                        isClearable
                                        isDisabled={!missingVehicleID}
                                        className={`${missingVehicleID ? 'requiredColor' : 'readonlyColor'}`}
                                        placeholder="Select..."
                                    />
                                </div>
                                <span className='' style={{ marginTop: '-8px' }}>
                                    <div className="text-field col-12 col-md-12 col-lg-12 ">
                                        <input
                                            className={`${value.PlateID ? "requiredColor" : ''} ${!value?.PlateID || !missingVehicleID ? 'readonlyColor' : ''}`}
                                            disabled={!value?.PlateID || !missingVehicleID}
                                            isDisabled={!value?.PlateID || !missingVehicleID}
                            ...
                            <div className="col-4 col-md-4 col-lg-3 mt-1 text-field d-flex">
                                <input type="text" name='VIN' id='VIN' maxLength={17} value={value?.VIN} onChange={HandleChanges} className={`${missingVehicleID ? '' : 'readonlyColor'}`} required autoComplete='off' disabled={!missingVehicleID} />
                            ...
                            <div className="col-4 col-md-4 col-lg-3 mt-1 text-field ">
                                <input type="text" name='OANID' id='OANID' disabled={!missingVehicleID}
                                    className={`${missingVehicleID ? '' : 'readonlyColor'}`} value={value?.OANID} onChange={HandleChanges} required maxLength={20} autoComplete='off' />
                            </div>
                            <div className="col-2 col-md-2 col-lg-1 mt-2 ">
                            ...
                            <div className="col-4 col-md-4 col-lg-3 mt-1 ">
                                <Select
                                    ...
                                    isClearable
                                    isDisabled={value?.CategoryID || missingVehicleID ? false : true}
                                    className={!value?.CategoryID || !missingVehicleID ? 'readonlyColor' : ''}
                                    placeholder="Select..."
                                />
                            </div>
                            ...
                            <div className="col-4 col-md-4 col-lg-3 mt-1 ">
                                <CreatableSelect
                                    ...
                                    isDisabled={!value?.MakeID || !missingVehicleID}
                            ...
                            <div className="col-4 col-md-4 col-lg-1 mt-1 text-field">
                                <input type="text" name='Weight' id='Weight' disabled={!missingVehicleID}
                                    className={`${missingVehicleID ? '' : 'readonlyColor'}`} maxLength={4} value={value?.Weight} onChange={HandleChanges} required autoComplete='off' />
                            </div>
                            <div className="col-2 col-md-2 col-lg-1 mt-2 ">
                            ...
                            <div className="col-4 col-md-4 col-lg-1 mt-1 text-field">
                                <input
                                    ...
                                    name="Value"
                                    id="Value"
                                    className={`${missingVehicleID ? (lossCode === 'STOL' || lossCode === 'BURN' || lossCode === 'RECD' ? 'requiredColor' : '') : 'readonlyColor'}`}
                                    maxLength={20}
                                    value={`$ ${value?.Value}`}
                            ...
                            <div className="col-4 col-md-4 col-lg-1 mt-1 text-field">
                                <input type="text" name='Inspection_Sticker'
                                    disabled={!missingVehicleID}
                                    className={`${missingVehicleID ? '' : 'readonlyColor'}`}
                                    id='Inspection_Sticker' value={value?.Inspection_Sticker} onChange={HandleChanges} required autoComplete='off' />
                            </div>
                            <div className="col-2 col-md-2 col-lg-2 mt-2 ">
                            ...
                            <div className="col-12 col-md-12 col-lg-12 mt-2">
                                <div className="d-flex align-items-center">
                                    ...
                                    <input type='text' name='otherIdentifyingCharacteristics' disabled={!missingVehicleID}
                                        className={`${missingVehicleID ? '' : 'readonlyColor'}` + ' form-control'} value={personNotifyUIForm.otherIdentifyingCharacteristics} onChange={handlePersonNotifyUIChange} placeholder='Placeholder' />
                                </div>
                            </div>
                        </div>
                    </div>
