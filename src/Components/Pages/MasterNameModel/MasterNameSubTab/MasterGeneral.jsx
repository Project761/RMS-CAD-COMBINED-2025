...
import { customStylesWithOutColor, Decrypt_Id_Name, DecryptedList, getShowingWithOutTime } from "../../../Common/Utility";
...
const MasterGeneral = (props) => {
    ...
    const { setChangesStatus, setMasterCountgenStatus } = useContext(AgencyContext);
    ...
    useEffect(() => {
        if (type === 'VehicleOwner' && ownerOfID) {
            ...
            get_Single_Data(type === "ComplainantName" ? complainNameID : possessionID, mstPossessionID);
        }
    ...
    useEffect(() => {
        if (editval) {
            ...
            setValue({
                ...value,
                'MasterNameID': mstPossessionID, 'NameID': type === "ComplainantName" ? complainNameID : possessionID,
            ...
            if (editval[0]?.DLStateID || editval[0]?.DLNumber || editval[0]?.MaritalStatusID || editval[0]?.HairColorID || editval[0]?.BIVerifyID
    ...
    const Update_Name = () => {
        ...
        const val = {
            ...
            "BICityID": BICityID, 'NameID': type == 'VehicleOwner' ? ownerOfID : type === "ComplainantName" ? complainNameID : possessionID, 'MasterNameID': mstPossessionID, 'PINID': PINID, 'CreatedByUserFK': loginPinID, 'ModifiedByUserFK': loginPinID, 'IsMaster': IsMaster,
        }
        AddDeleteUpadate('MasterName/UpdateBasicInfoName', val).then((res) => {
            ...
            get_Single_Data(type === "ComplainantName" ? complainNameID : possessionID || ownerOfID, mstPossessionID);
