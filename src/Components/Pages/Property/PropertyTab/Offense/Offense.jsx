...
import { check_OffenceCode_NoneUnknown, TableErrorTooltip } from '../../PropertyNibrsError';
...
const Offense = (props) => {
  ...
  useEffect(() => {
    if (localStoreData) {
      ...
      dispatch(get_ScreenPermissions_Data("P063", localStoreData?.AgencyID, localStoreData?.PINID));
    }
  }, [localStoreData]);
  ...
  useEffect(() => {
    if (DecPropID || DecMPropID) {
      get_Data_Owner(DecPropID, DecMPropID, DecIncID);
      setValue({
        ...value,
        'IncidentID': DecIncID, 'PropertyID': DecPropID, 'CreatedByUserFK': loginPinID, 'MasterPropertyID': DecMPropID, 'labal': '', 'OffenseID': null,
      })
    }
  }, [DecPropID, DecIncID]);
  ...
  const get_Data_Owner = (propertyID, DecMPropID, mainIncidentID) => {
    ...
    fetchPostData('PropertyOffense/GetData_PropertyOffense', val).then((res) => {
      if (res) {
        setOwnerData(res);
        setCategoryCode(res[0]?.PropDescCode);
    ...
    fetchPostData('PropertyOffense/GetDataDropDown_PropertyOffense', val).then((data) => {
      if (data) {
        ...
        if (ListData[0]?.PropertyCategory_Description === "Drugs/Narcotics") {
          ...
          const filterArray = data?.filter((item) => item.FBICode === '35A')
          setFilterData(shortArr(ownerData, filterArray));
          setOwnerIdDrp(shortArr(ownerData, filterArray));



        } else if (ListData[0]?.PropertyCategory_Description === "Drug/Narcotic Equipment") {

          const filterArray = data?.filter((item) => item.FBICode === '35B')
          setFilterData(shortArr(ownerData, filterArray));
          setOwnerIdDrp(shortArr(ownerData, filterArray));
  ...
  const Add_Owner = () => {
    ...
    if (result) {
      ...
      const val = {
        'IncidentID': DecIncID, 'PropertyID': DecPropID, 'CreatedByUserFK': loginPinID, 'IsMaster': IsMaster,
        'MasterPropertyID': DecMPropID, 'labal': labal, 'OffenseID': OffenseID,
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "140px",
      cell: (row) => (
        check_OffenceCode_NoneUnknown(row.NIBRSCode, propLossCode, row.AttemptComplete, categoryCode) ? <TableErrorTooltip value={row.NIBRSCode} ErrorStr={check_OffenceCode_NoneUnknown(row.NIBRSCode, propLossCode, row.AttemptComplete, categoryCode)} /> : row.NIBRSCode
      ),
    },
    {
  ...
  const DeletePin = () => {
    ...
    AddDeleteUpadate('PropertyOffense/Delete_PropertyOffense', val).then((res) => {
      if (res) {
        ...
        const message = parsedData.Table[0].Message;
        toastifySuccess(message);
        get_Property_Count(DecPropID, DecMPropID, MstPage === "MST-Property-Dash" ? true : false); setDeleteStatus(false);
        get_OwnerID_Drp(DecIncID); get_Data_Owner(DecPropID, DecMPropID, DecIncID);
        onClear();
  ...
  const conditionalRowStyles = [
    {
      when: () => true,
      style: (row) => getStatusColors(row.NIBRSCode, row.AttemptComplete, row.PropDescCode),
    },
  ];

  const getStatusColors = (Code, AttmComp, categoryCode) => {
