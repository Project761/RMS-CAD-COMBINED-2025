...
const Home = ({ setShowJuvinile, setEditval, setShowPage, setResetErrors, setShowPoliceForce, ResetErrors, DecArrestId, setStatus, isEnabled, setIsEnabled, Agencystatus, setAgencystatus, arrestID, setArrestID, setmatchedAgency, ChargeLocalArr, possessionID, setPossessionID, RestStatus, Editval, incExceDate, GetSingleData, get_List, isLocked, }) => {
    ...
    var localStatus = query?.get("localStorageStatus");
    ...
    const [localArrData, setlocalArrData] = useState();
    ...
    useEffect(() => {
        if (Editval?.length > 0 || localArrData) {
            ...
            setArrestName((Editval[0]?.Arrestee_Name ? Editval[0]?.Arrestee_Name : '') || (Editval[0]?.Arrestee_Name ? Editval[0]?.Arrestee_Name : ''));
            setIncidentNumber((Editval[0]?.IncidentNumber ? Editval[0]?.IncidentNumber : '') || (localArrData?.IncidentNumber ? localArrData?.IncidentNumber : '')); setArrestDate((Editval[0]?.ArrestDtTm ? new Date(Editval[0]?.ArrestDtTm) : '') || (localArrData?.ArrestDtTm ? new Date(localArrData?.ArrestDtTm) : '')); setRightGivenCode(Get_Given_Code(Editval, policeForceDrpData));

            if ((Editval[0]?.PoliceForceID === 1 || Editval[0]?.PoliceForceID === '1') || (localArrData?.PoliceForceID === 1 || localArrData?.PoliceForceID === '1')) {
                setShowPoliceForce(true);
            } else if ((Editval[0]?.PoliceForceID === 2 || Editval[0]?.PoliceForceID === '2') || (localArrData?.PoliceForceID === 2 || localArrData?.PoliceForceID === '2')) {
                setShowPoliceForce(false);
            }
            if ((Editval[0]?.IsJuvenileArrest === true || Editval[0]?.IsJuvenileArrest === 'true') || (localArrData?.IsJuvenileArrest === true || localArrData?.IsJuvenileArrest === 'true')) {
                setShowJuvinile(true);
            } else {
                setShowJuvinile(false);
            }
            if ((Editval[0]?.PoliceForceID === 1) || (localArrData?.PoliceForceID === 1)) {
                setIsEnabled(true);
