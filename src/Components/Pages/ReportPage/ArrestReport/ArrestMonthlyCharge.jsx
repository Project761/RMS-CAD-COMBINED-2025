...
const ArrestMonthlyCharge = () => {
    ...
    const [value, setValue] = useState({
        NIBRSID: null, ChargeCodeID: null, ArrestDtTm: '', AgencyID: '',
    });
    ...
    useEffect(() => {
        if (localStoreData) {
            setLoginAgencyID(localStoreData?.AgencyID); setLoginPinID(localStoreData?.PINID); FBIidDrpVal(localStoreData?.AgencyID);
        }
    }, [localStoreData])
    ...
    const getIncidentSearchData = async () => {
        if (value?.ArrestDtTm.trim()?.length > 0 || value?.NIBRSID !== null || value?.ChargeCodeID !== null) {
            const { NIBRSID, ChargeCodeID, ArrestDtTm, AgencyID, } = value
            const val = {
                NIBRSID: NIBRSID, ChargeCodeID: ChargeCodeID, ArrestDtTm: ArrestDtTm, AgencyID: LoginAgencyID,
            }
            // console.log(value)
            fetchPostData('ArrestReport/ArrestMonthlyByChargeReport', val).then((res) => {
                if (res.length > 0) {
                    // console.log(res)
    ...
    const resetFields = () => {
        setValue({ ...value, NIBRSID: null, ChargeCodeID: null, ArrestDtTm: '', });
